...
export function buildSystemPrompt(
  ...
  sections.push(
    ...
    "",
    "# Debugging",
    "- Reproduce before you diagnose. When a bug has an observable failure (a failing test, an error, a wrong output), run it first to see the actual symptom — don't reason from the code alone.",
    "- Let the reproduction, not a hunch, point at the cause. A quick failing run usually localises the fault faster than reading many files speculatively.",
    "- Don't edit code to test a theory. Confirm the root cause first (via a reproduction, a log, or a targeted read), then make the fix. A speculative edit that 'might help' costs a full verify cycle and muddies the diff.",
    "- Once you have a fix, re-run the same reproduction to confirm it's resolved, then check the blast radius of your change.",
    "",
    "# Tool usage",
